...
function StatusBadge({ status }: { status: string }) {
  if (status === "full")
    return (
      <span className="rounded-md bg-red-500/20 px-1.5 py-0.5 text-[10px] font-bold text-red-300 ring-1 ring-red-500/40">
        เต็ม
      </span>
    );
  if (status === "warning")
...
export default function KeyPage() {
  ...
  const [limitsConfig, setLimitsConfig] = useState<RiskLimitsConfig | null>(
    null,
  );
    ...
    try {
      ...
      setMessage(
        `บันทึก ${data.added} รายการ` +
          (data.skipped > 0 ? ` · ข้ามเลขเต็ม ${data.skipped} รายการ` : ""),
      );
  ...
  async function handleNewDraw() {
    if (!confirm("ปิดงวดเดิมและเปิดงวดใหม่?")) return;
  ...
  const capForDialog = capNumber && limitsConfig
    ...
    <>
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-slate-400">
            {summary?.draw.label ?? "—"} · อัปเดตอัตโนมัติทุก 4 วินาที
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
        <textarea
          ...
          placeholder={`วางจาก LINE...\n2476\n8210\n20 ชุด`}
          rows={8}
          className="w-full resize-y rounded-xl border border-white/10 bg-slate-950/80 px-3 py-3 font-mono text-sm text-amber-50 outline-none focus:ring-2 focus:ring-amber-500/40"
        />
        ...
        <>
          <section className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
            {[
              ...
              {
                l: "ยอดรับ",
                v: `฿${summary.totals.totalReceived.toLocaleString()}`,
              },
              {
                l: "เสี่ยง",
                v: `฿${summary.totals.totalRisk.toLocaleString()}`,
                w: true,
              },
              ...
              <div
                ...
                className={`rounded-2xl border p-3 text-center ${
                  c.w ? "border-red-500/30 bg-red-950/30" : "border-white/10 bg-white/5"
                }`}
              >
          ...
          <section className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-white/5">
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-slate-900 text-[10px] uppercase text-slate-500">
                  <tr>
                    ...
                    <th className="px-2 py-2 text-right">เสี่ยง</th>
                    <th className="px-2 py-2">เพดาน</th>
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr
                      ...
                      className={`cursor-pointer border-t border-white/5 hover:bg-amber-500/10 ${
                        row.status === "full" ? "bg-red-950/25" : ""
                      }`}
                    >
                      <td className="px-2 py-3 font-mono font-bold text-amber-200">
                        {row.number}
                      </td>
                      ...
                      <td className="px-2 py-3 text-right tabular-nums text-red-300/90">
                        {row.maxPayout.toLocaleString()}
                      </td>
            ...
            <p className="px-4 py-2 text-[10px] text-slate-500">
              กดแถวตั้งเพดาน · เลขเต็มจะไม่รับโพยเพิ่ม
            </p>
          </section>
        </>
      )}
      ...
      {capNumber && limitsConfig && (
        <SetCapDialog
          ...
          onClear={async () => {
            await fetch(`/api/limits?number=${capNumber}`, {
              method: "DELETE",
            });
            setCapNumber(null);
            await loadSummary();
